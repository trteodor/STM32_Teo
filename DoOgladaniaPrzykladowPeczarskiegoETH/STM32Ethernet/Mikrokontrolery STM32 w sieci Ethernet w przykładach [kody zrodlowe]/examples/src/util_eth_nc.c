...
#define TX_BUFFER_SIZE (ETH_MAX_PACKET_SIZE + VLAN_TAG - ETH_CRC)
...
void ETHconfigureDMA(struct netif *netif,
  ...
  for (i = 0; i < ETH_TXBUFNB; ++i) {
    ...
    DMAtxRing[i].Buffer1Addr = (uint32_t)TxBuff[i];
  ...
  /* Zgłaszaj przerwanie po odebraniu ramki. */
...
err_t low_level_output(struct netif *netif, struct pbuf *p) {
  ...
  if (DMAtxRing[DMAtxIdx].Status & ETH_DMATxDesc_OWN) {
    ((struct ethnetif *)netif->state)->TX_errors++;
    return ERR_IF; /* Błąd, deskryptor jest zajęty. */
  }

  /* Implementacja zero-copy wysyłania ramek jest niemożliwa, gdy
     chcemy wysyłać dane z pamięci nieulotnej, np. FLASH. DMA nie
     potrafi kopiować z takiej pamięci. */
  pbuf_copy_partial(p, (void *)DMAtxRing[DMAtxIdx].Buffer1Addr,
                    p->tot_len, 0);
  DMAtxRing[DMAtxIdx].ControlBufferSize =
                                p->tot_len & ETH_DMATxDesc_TBS1;
  DMAtxRing[DMAtxIdx].Status |= ETH_DMATxDesc_FS |
                                ETH_DMATxDesc_LS |
                                ETH_DMATxDesc_OWN;
